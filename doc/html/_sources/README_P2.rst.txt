...
========
        ...
        :``./src/p2/types_and_ops.h``:        Enumerations & conversion functions used with GPL language types and operators.
        :``./src/p2/types_and_ops.cpp``:      Source to conversion functions declared in header file.
...
1.  Copy the following files over from ``p1`` into ``p2``:
...
3.  For this second project, three kinds of symbols are managed: strings, integers, and doubles. To facilitate this, declare a union of the three possible types as private data member of a ``Symbol`` class (see following step). Each member of the union is a pointer to the type it manages. Creating a constructor for each union member eases its use in ``Symbol``'s initialization list. Example of union declaration:
...
4.  Write a ``Symbol`` class.  
    ...
    * The data member ``value`` points to the actual value (int, double, or string) or array of same. The value is created using ``new`` outside of the constructor. That is, the allocation using ``new`` occurs prior to creating/declaring the Symbol object. The constructor takes a pointer to the newly-allocated value(s).
    * The destructor (``Symbol::~Symbol``) is responsible for releasing the memory (``delete``, or ``delete []``, depending on whether value points to a single variable or array of variables). A fair amount of branching (if/elses and/or switches) will be required to properly release the memory based on its type and whether it is an array.
    * The insertion operator (``operator<<``) should print out the symbol in the following format::
      ...
      The ``to_string()`` function can be useful here (see ``types_and_ops.(h|cpp)``)
...
9.  Write a test program to ensure ``Scope_manager`` is bug-free. Run the test code through ``valgrind`` to ensure it is free of memory leaks. Two modifications to the previous test program should be sufficient::
...
---------------------------------------------
Implement variable declarations in the parser
---------------------------------------------

Now that ``Symbol``, ``Symbol_table``, and ``Scope_manager`` have been developed, they will be used to implement variable declarations in the parser.

10.  In the ``simple_type:`` production in ``gpl.y``, each rule should set the production's semantic value of to the corresponding ``GPL::Type``:
    ...
    .. raw:: html
        ...
        <pre>simple_type:
            ...
            | T_STRING <span style="font-weight:bold;color:DarkRed;">{$$=GPL::STRING;}</span></pre>
...
11. Add ``Scope_manager.h`` to the header files included near the top of ``gpl.y``.

|

12. Assigning a ``GPL::Type`` to ``simple_type`` requires two additional configurations:

    1. Add ``GPL::Type union_gpl_type;`` to the ``%union`` declaration.
...
14. At this point the project can be built (``make``) and tested (``../../bin/gt``). Several tests will fail.  
    ...
    These warnings indicate that the enumeration type ``GPL::Type`` has many possible values not handled by the cases in switch statements. These warnings can be silenced by adding a default case::
...
16. Running the project tests reveals that several fail because required error-checking hasn't been added yet. Use ``vde`` to check error output, inspect the content of the ``tests/t###.gpl`` files directly to see why the test's GPL code generates an error, and consult ``error.cpp`` for the enumeration that corresponds to the expected error. Example of error-handling code:
    ...
    .. code-block:: cpp
       ...
       if(scopemgr.defined_in_current_scope(*$2))
       {
         ...
         // embeds this action in a switch-statement case
         break; 
       }
...
17. After these actions are written including error handling, all of the tests will pass. The next step is to eliminate memory leaks. Use ``valgrind`` to manually check individual tests for leaks. Use ``memvalidate.bash`` to check all tests (akin to the ``gt`` utility) for memory leaks. Subsequent steps describe sources of memory leaks.
...
18. The scanner (``gpl.l``) dynamically allocates strings for the semantic values of ``T_ID``. The parser needs to release this memory to avoid memory leaks. At the end of each action for ``variable_declaration``, release the memory allocated for ``T_ID``:
    ...
    Errors that ``break`` out of the action (Shown in the example on step 16), should release the memory prior to the ``break``.
...
* Changed Makefile to silence some warnings. This allows removal of section that describes how to change code to suppress warnings.
...
* Miscellaneous minor text elaborations and corrections

Ver. 28 September 2021
  
* In code sample for step 9, added to replace ``insert()`` with ``add_to_current_scope()``
